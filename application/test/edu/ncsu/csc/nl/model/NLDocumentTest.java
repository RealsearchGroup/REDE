...
public class NLDocumentTest {
	...
	public void testBasicSentenceCreation() {
		...
		assertEquals("Number of access control items = 0", s.getNumberOfAccessControlRelations(), 0);
	...
	@Test
	public void testSerializingfilesWithSOA() {
		// tests that children are processed first, such that we can rollup changes
		AccessControlRelationManager.getTheAccessControlRelationManager().reset(true);
		NLDocument document = TestConfiguration.createBasicDocumentForSOA();
		Sentence s = document.getElementAt(3);
		s.getSecurityObjectiveAnnotations().add(TestConfiguration.createSOA_ACCESS_CONTROL());
		s.getSecurityObjectiveAnnotations().add(TestConfiguration.createSOA_CONFIDENTIALITY());
		...
		TestConfiguration.validateBasicDocumentForSOA(document);
		Sentence s1 = document.getElementAt(3);
		TestConfiguration.validateSOA_ACCESS_CONTROL(s1.getSecurityObjectiveAnnotations().get(0));
		TestConfiguration.validateSOA_CONFIDENTIALITY(s1.getSecurityObjectiveAnnotations().get(1));
	}	
	...
	@Test
	public void testJSONFilesWithSecurityObjectiveAnnotation() {
		// tests that children are processed first, such that we can rollup changes
		AccessControlRelationManager.getTheAccessControlRelationManager().reset(true);
		NLDocument document = TestConfiguration.createBasicDocumentForSOA();
		Sentence s = document.getElementAt(3);
	...
	@Test
	public void testProduceSecurityObjectAnnotationReport() {
	}
