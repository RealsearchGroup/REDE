...
public class InstanceLearner {
	...
	protected void tallySecuritObjectiveAnnotationVotes(Sentence s, HashMap<String, Integer> votes) {
		SecurityObjective[] objectives = SecurityObjective.getSelectableList();
		for (SecurityObjective obj: objectives) {
			SecurityObjectiveAnnotation soa = s.getSecurityObjectiveAnnotation(obj);
			if (soa != null) {
				incrementHashMapValueStringKey(votes,obj.getAbbreviation()+"_yes");
				incrementHashMapValueStringKey(votes,obj.getAbbreviation()+"_impact_"+soa.getSecurityImpact());
				incrementHashMapValueStringKey(votes,obj.getAbbreviation()+"_implied_"+soa.isImplied());
				incrementHashMapValueStringKey(votes,obj.getAbbreviation()+"_mitigation_"+soa.getSecurityMitigation());	
			}
			else {
				incrementHashMapValueStringKey(votes,obj.getAbbreviation()+"_no");
			}
		}		
	}
	...
	protected void findSecurityObjectiveAnnotationMajority(HashMap<String, Integer> votes, ClassificationResult result) {
		SecurityObjective[] objectives = SecurityObjective.getSelectableList();
		for (SecurityObjective obj: objectives) {
			String positiveKey = obj.getAbbreviation()+"_yes";
			String negativeKey = obj.getAbbreviation()+"_no";
			
			if ( getDefaultVoteValue(votes,positiveKey) >=  getDefaultVoteValue(votes,negativeKey)) {
				SecurityObjectiveAnnotation soa = new SecurityObjectiveAnnotation();
				soa.setSecurityObjective(obj);
				soa.setSecurityImpact(this.getSecurityImpactWithGreatestVote(obj, votes));
				soa.setSecurityMitigation(this.getSecurityMitigationWithGreatestVote(obj, votes));
				
				String impliedTrueKey  = obj.getAbbreviation()+"_implied_"+true;
				String impliedFalseKey = obj.getAbbreviation()+"_implied_"+false;
				
				soa.setImplied(  getDefaultVoteValue(votes,impliedTrueKey) >=   getDefaultVoteValue(votes,impliedFalseKey)  );
				
				result.annotations.add(soa);
			}
		}		
	}
	...
	protected HashMap<String, Integer> computeClassificationVotes(IntermediateResults[] distances, int k, ClassificationResult result) {
		...
		for (int i=0; i<=finalSortedIndexMax; i++) {
			...
			tallySecuritObjectiveAnnotationVotes(s,votes);
	...
	public ClassificationResult getClassification(Sentence testSentence, int k, boolean verbose, boolean skipTestSentences, boolean sendEvent,SentenceDistance distanceAlgorithm) {		
		...
		this.findSecurityObjectiveAnnotationMajority(votes, result);
	...
	protected SecurityImpact getSecurityImpactWithGreatestVote(SecurityObjective obj, HashMap<String,Integer> votes) {
		SecurityImpact result = SecurityImpact.NOT_APPLICABLE;
		
		String naKey   = obj.getAbbreviation()+"_impact_"+SecurityImpact.NOT_APPLICABLE;
		String lowKey  = obj.getAbbreviation()+"_impact_"+SecurityImpact.LOW;
		String modKey  = obj.getAbbreviation()+"_impact_"+SecurityImpact.MODERATE;
		String highKey = obj.getAbbreviation()+"_impact_"+SecurityImpact.HIGH;
		
		if ( getDefaultVoteValue(votes,lowKey) >=  getDefaultVoteValue(votes,naKey) &&  getDefaultVoteValue(votes,lowKey) >=  getDefaultVoteValue(votes,modKey) &&  getDefaultVoteValue(votes,lowKey) >=  getDefaultVoteValue(votes,highKey) ) {
			result = SecurityImpact.LOW;
		}
		else if ( getDefaultVoteValue(votes,modKey) >=  getDefaultVoteValue(votes,naKey) &&  getDefaultVoteValue(votes,modKey) >=  getDefaultVoteValue(votes,lowKey) &&  getDefaultVoteValue(votes,modKey) >=  getDefaultVoteValue(votes,highKey) ) {
			result = SecurityImpact.MODERATE;
		}
		else if ( getDefaultVoteValue(votes,highKey) >=  getDefaultVoteValue(votes,naKey) &&  getDefaultVoteValue(votes,highKey) >=  getDefaultVoteValue(votes,lowKey) &&  getDefaultVoteValue(votes,highKey) >=  getDefaultVoteValue(votes,modKey) ) {
	...
	protected SecurityMitigation getSecurityMitigationWithGreatestVote(SecurityObjective obj, HashMap<String,Integer> votes) {
		SecurityMitigation result = SecurityMitigation.ADAPTATION;
		
		String adptKey = obj.getAbbreviation()+"_mitigation_"+SecurityMitigation.ADAPTATION;
		String dtctKey = obj.getAbbreviation()+"_mitigation_"+SecurityMitigation.DETECTION;
		String prvtKey = obj.getAbbreviation()+"_mitigation_"+SecurityMitigation.PREVENTION;
		String rectKey = obj.getAbbreviation()+"_mitigation_"+SecurityMitigation.REACTION;
		
		if ( getDefaultVoteValue(votes,dtctKey) >=  getDefaultVoteValue(votes,adptKey) &&  getDefaultVoteValue(votes,dtctKey) >=  getDefaultVoteValue(votes,prvtKey) &&  getDefaultVoteValue(votes,dtctKey) >=  getDefaultVoteValue(votes,rectKey) ) {
			result = SecurityMitigation.DETECTION;
		}
		else if ( getDefaultVoteValue(votes,prvtKey) >=  getDefaultVoteValue(votes,adptKey) &&  getDefaultVoteValue(votes,prvtKey) >=  getDefaultVoteValue(votes,dtctKey) &&  getDefaultVoteValue(votes,prvtKey) >=  getDefaultVoteValue(votes,rectKey) ) {
			result = SecurityMitigation.PREVENTION;
		}
		else if ( getDefaultVoteValue(votes,rectKey) >=  getDefaultVoteValue(votes,adptKey) &&  getDefaultVoteValue(votes,rectKey) >=  getDefaultVoteValue(votes,dtctKey) &&  getDefaultVoteValue(votes,rectKey) >=  getDefaultVoteValue(votes,prvtKey) ) {
			result = SecurityMitigation.REACTION;
		}
	 ...
	 * 
	 */
	public ConfusionMatrix performEvaluationForAnnotations(ExpirementOptions options, java.util.ArrayList<java.util.ArrayList<Document>> folds ) {
		boolean verbose = false;
		SecurityObjective primaryObjective = options.getPrimarySecurityObjective();
		
		int numberOfTrainedSentences   = _trainedSentences.size();
		int numberOfPrimarySentences    = DocumentFoldCreator.countNumberOfSentencesByAnnotation(_trainedSentences, primaryObjective);
		...
		for (int i=0;i<numberOfFolds;i++) {
			...
			for (Document d:fold) {
				...
				boolean classificationResultForObjective = r.hasSecurityObjective(primaryObjective);
				
				if (r.averageDistance > d.sentence.getNumberOfNodes() * threshold) {
					try { 
					  foldUnknownMatrix.setCell(d.sentence.hasSecurityObjectiveAnnotation(primaryObjective), classificationResultForObjective);
					}
					catch (Exception e) {
						System.err.println("Classification not defined (assuming null): "+primaryObjective+", "+d.sentence);
						foldUnknownMatrix.setCell(false, false);
					}					
				}
				else {
					try{
						foldMatrix.setCell(d.sentence.hasSecurityObjectiveAnnotation(primaryObjective), classificationResultForObjective);
										
						if ( verbose && d.sentence.hasSecurityObjectiveAnnotation(primaryObjective) !=classificationResultForObjective) {
							System.out.println ("Mistake("+primaryObjective+","+d.sentence.hasSecurityObjectiveAnnotation(primaryObjective)+"): "+d.sentence);
							...
							for (int kNum=0; kNum <kMax; kNum++) {
								...
								System.out.println("\t"+kNum+":(distance="+r.distances[kNum].distance+", "+ sNeighbor.hasSecurityObjectiveAnnotation(primaryObjective)+": "+sNeighbor);
						...
						System.err.println("SecurityObjectiveAnnotation not defined (assuming null): "+primaryObjective);
		...
		System.out.println("Test Paramaters: k="+k+", n="+numberOfFolds+", threshold="+threshold+", objective="+primaryObjective);
	 ...
	 * 
	 */
	public ConfusionMatrix performEvaluationForAnnotationsByFolds(ExpirementOptions options ) {
		boolean verbose = false;
		SecurityObjective primaryObjective = options.getPrimarySecurityObjective();
		...
		java.util.ArrayList<java.util.ArrayList<Document>> folds = DocumentFoldCreator.createFoldsByObjective(_trainedSentences, primaryObjective, numberOfFolds);

		ConfusionMatrix totalMatrix = this.performEvaluationForAnnotations(options, folds);
	 ...
	 * 
	 */
	public ConfusionMatrix performEvaluationForAnnotationsByDocuments(ExpirementOptions options) {
		java.util.ArrayList<java.util.ArrayList<Document>> folds = DocumentFoldCreator.createFoldsByDocumentID(_trainedSentences);
		ConfusionMatrix totalMatrix = performEvaluationForAnnotations(options, folds);
