...
public class SentenceDisplayPanel extends JPanel implements ActionListener, NLPEventListener {
	...
	public void setCurrentSentence(int modelIndex, boolean setObject) {
		...
		_jcbConditions.setSelected(s.isAccessControlConditionPresent());
	...
	public void actionPerformed(ActionEvent ae) {
		...
		else if (ae.getSource() == _jcbConditions) {
			_currentSentence.setAccessControlConditionPresent(_jcbConditions.isSelected());
		}
	...
	public void eventOccured(NLPEventType eventType, NLPEvent event) {   
		if (eventType == NLPEventType.VIEW_CHANGED) {  // swapping between annotations and classifications in the table
			NLPEventViewChanged e = (NLPEventViewChanged) event;
			if (e.getCurrentView() == GCConstants.VIEW_ANNOTATIONS) {
				_jtSentence.getColumnModel().getColumn(2).setHeaderValue("SOA");
			}
			else if (e.getCurrentView() == GCConstants.VIEW_CLASSIFICATIONS) {
				_jtSentence.getColumnModel().getColumn(2).setHeaderValue("Classifications");
			}
			_jtSentence.getTableHeader().repaint();
			_jtSentence.repaint();
		}
		else if (eventType == NLPEventType.SENTENCE_DATA_CHANGE){
