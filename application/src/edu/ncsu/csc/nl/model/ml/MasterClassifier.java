...
public class MasterClassifier {
	...
	private java.util.List<SecurityObjective> _annotationList;                  // what are the different security objectives for annotations that can be classified
	...
	public MasterClassifier( java.util.List<ClassificationAttribute> classificationList, java.util.List<SecurityObjective> annotationList) {
	...
	public void createWekaInterfacer(String documentID) {
		...
		Instances dataSetAnnotations     = (new WekaCreator()).createWekaInstancesForSecurityObjectiveAnnotations(documentID,sentences,wo, _annotationList);
		_wekaInterfacer = new WekaInterfacer();
		_wekaInterfacer.trainClassifiers(dataSetClassifications, _classificationAttributes, dataSetAnnotations, _annotationList);
	}
	...
	public void performOverallSelfEvaluate() {
		...
		for (ClassificationAttribute ca: _classificationAttributes) {
			...
			ClassifierOptions options  = new ClassifierOptions( ca,null,ClassifierOptions.FOLD_RANDOM, WordType.LEMMA, 10, false, StopWord.getListByName(StopWord.DETERMINER), false);
	...
	public void performOverallSelfEvaluateForAnnotations() {
		System.out.println("Ensuring classifiers are created");
		this.createWekaInterfacer("genericID");
		...
		for (SecurityObjective so: _annotationList) {
			System.out.print("Starting: "+so);
			String primaryAnnotation = so.toString();
			numberOfAttributesEvaluted++;
			
			ClassifierOptions options  = new ClassifierOptions(null, so,ClassifierOptions.FOLD_RANDOM, WordType.LEMMA, 10, false, StopWord.getListByName(StopWord.DETERMINER), false);
			this.setClassifierOptions(options);
			ConfusionMatrix attributeMatrix = new ConfusionMatrix();
			
			//java.util.ArrayList<java.util.ArrayList<Document>> folds = DocumentFoldCreator.createFoldsByObjective(documents,so, options.getNumberOfFolds());
			java.util.ArrayList<java.util.ArrayList<Document>> folds = DocumentFoldCreator.createFoldsByDocumentID(documents);
			...
			for (int i=0;i<numberOfFolds;i++) {
				...
				for (Document d:fold) {
					...
					Instances testDataSetSentence = (new WekaCreator()).createWekaInstancesForSecurityObjectiveAnnotations("testFold",sentences,wo,_annotationList);

					ClassificationResult kNNResult = this.getInstanceLearner().getClassification(d.sentence, k, false, true,false, sentenceDistance);  // instance learner classify both at the same time
					ClassificationResult wekaNB     = this.getWekaInterfacer().classifyAnnotationsWithNaiveBayesMultinomial(testDataSetSentence,so);
					ClassificationResult wekaSMO    = this.getWekaInterfacer().classifyAnnotationsWithSMO(testDataSetSentence,so);

					
					boolean instanceResult = kNNResult.hasSecurityObjective(so);
					
					if (kNNResult.averageDistance/d.sentence.getNumberOfNodes() > .6) {
						int foundTrue = 0;
						if (instanceResult) { foundTrue++; }
						if ( wekaNB.hasSecurityObjective(so))  {  foundTrue++; }
						if ( wekaSMO.hasSecurityObjective(so)) {  foundTrue++; }
						if (foundTrue == 3) {
							foldMatrix.setCell(d.sentence.hasSecurityObjectiveAnnotation(so), true);
						}
						else if (foundTrue ==0) {
							foldMatrix.setCell(d.sentence.hasSecurityObjectiveAnnotation(so), false);
						}
						else {
							foldMatrix.setCell(d.sentence.hasSecurityObjectiveAnnotation(so), wekaSMO.hasSecurityObjective(so));
						}

					}
					else {
						foldMatrix.setCell(d.sentence.hasSecurityObjectiveAnnotation(so), instanceResult);
					}
					/*
					if ( d.sentence.hasBooleanClassification(primaryClassification) != bc.getValue().getBooleanValue()) {
						//System.out.println ("Mistake("+primaryClassification+",actual="+s.hasBooleanClassification(primaryClassification)+"): "+s);
					...
					*/
					...
					if (verbose) {
						System.out.print(so.getAbbreviation());
						System.out.print("\t");
						System.out.print(d.sentence);
						System.out.print("\t");					
						System.out.print(d.sentence.hasSecurityObjectiveAnnotation(so));
						System.out.print("\t");
						System.out.print(kNNResult.hasSecurityObjective(so));
						System.out.print("\t");
						System.out.print(kNNResult.averageDistance);
						System.out.print("\t");
						System.out.print( wekaNB.hasSecurityObjective(so));
						System.out.print("\t");			
						System.out.print(this.getRatio( wekaNB.distributions.get(primaryAnnotation)));
						System.out.print("\t");
						System.out.print(wekaSMO.hasSecurityObjective(so));
						System.out.print("\t");			
						System.out.print(this.getRatio( wekaSMO.distributions.get(primaryAnnotation)));
			...
			//average the results, and report
			System.out.println("Test Paramaters: k="+k+", n="+numberOfFolds+", threshold="+threshold+", attribute="+primaryAnnotation);
			System.out.println("Sentence Distance Method: "+sentenceDistance.getMethodName());
			System.out.println("TOTAL:  "+ attributeMatrix);
