...
public class WekaInterfacer implements Serializable {
	...
	private HashMap<SecurityObjective, Classifier> _naiveBayesClassifierForAnnotations   = new HashMap<SecurityObjective, Classifier>();
	private HashMap<SecurityObjective, Classifier> _smoClassifierForAnnotations = new HashMap<SecurityObjective, Classifier>();
	...
	public void trainClassifiers(Instances dataSet, java.util.List<ClassificationAttribute> attributeList, 
			                     Instances annotationDataSet, java.util.List<SecurityObjective> annotationList) {
		...
		// Now train the classifiers for the security objective annotation list
		for (SecurityObjective so: annotationList) {
			
			Instances currentDataSet = removeOtherAnnotations(annotationDataSet, so);
			currentDataSet = this.convertSentenceToWordVector(currentDataSet, true, false, "at_"+so.toString());
			currentDataSet.setClassIndex(this.getAnnotationPosition(currentDataSet,so));
			
			try {				
				NaiveBayesMultinomial nbm = new NaiveBayesMultinomial();
				nbm.buildClassifier(currentDataSet);
				
				_naiveBayesClassifierForAnnotations.put(so, nbm);

				weka.classifiers.functions.SMO smo = new weka.classifiers.functions.SMO(); 
				smo.buildClassifier(currentDataSet);
				_smoClassifierForAnnotations.put(so,smo);
	...
	private Instances removeOtherAnnotations(Instances dataSet, SecurityObjective annotationToTest) {
		int numberOfAttributes = dataSet.numAttributes();
		for (int i=numberOfAttributes-1;i>=0;i--) {
			//System.out.println(dataSet.attribute(i));
			if (dataSet.attribute(i).name().equals("sentence") || dataSet.attribute(i).name().equals(annotationToTest.toString())) {continue; }
	...
	private ConfusionMatrix evaluateNFoldWekaClassifierForAnnotations(Instances completeDataSet, java.util.List<SecurityObjective> attributeList, weka.classifiers.Classifier classifier, int numberOfFolds)  {		
		ConfusionMatrix totalMatrix = new ConfusionMatrix();
		double numberOfAttributesEvaluted = 0.0;
		double precision = 0.0;
		double recall    = 0.0;
		...
		for (SecurityObjective so: attributeList) {
			numberOfAttributesEvaluted++;
			
			Instances currentDataSet = this.removeOtherAnnotations(completeDataSet, so);
			currentDataSet = this.convertSentenceToWordVector(currentDataSet, false, false, null);		
			...
			try {
				...
				System.out.println(so +":" + attributeMatrix);
				
			}
			catch (Exception e) {
				System.err.println(so+": WekaInterfacer.classifyEvaluation: "+e);
			}
	...
	public ConfusionMatrix evaluateNFoldWithNaiveBayesMultinomialForAnnotations(Instances dataSet, java.util.List<SecurityObjective> attributeList, int numberOfFolds) throws Exception {
		System.out.println("Weka - NaiveBayes:");
		
		return evaluateNFoldWekaClassifierForAnnotations(dataSet,attributeList, new NaiveBayesMultinomial(), numberOfFolds);

	}

	public ConfusionMatrix evaluateNFoldWithSupportVectorMachineForAnnotations(Instances dataSet, java.util.List<SecurityObjective> attributeList, int numberOfFolds) throws Exception {
		System.out.println("Weka - Support Vector Machine (SMO):");
		
		return evaluateNFoldWekaClassifierForAnnotations(dataSet,attributeList, new weka.classifiers.functions.SMO(),numberOfFolds);
	}
	...
	private int getAnnotationPosition(Instances dataSet, SecurityObjective attributeToTest) {
		int classPosition = -1;
		
		for (int i=0;i<dataSet.numAttributes();i++) {
			if (dataSet.attribute(i).name().equals( attributeToTest.toString())) { classPosition = i; }
		}	
	...
	private ClassificationResult classifyAnnotations(HashMap<SecurityObjective, Classifier> classifierMap, Instances originalDataSet, SecurityObjective soToTest) {
		ClassificationResult result = new ClassificationResult();
		result.classificationMethod = ClassificationResult.WEKA_NAIVE_BAYES;
		
		for (SecurityObjective attribute: classifierMap.keySet()) {
			if (soToTest != null && soToTest != attribute ) { continue; }   // if we only care about one item, just test that!
			Instances dataSet = originalDataSet;
			Classifier classifier = classifierMap.get(attribute);
			
			try {
				dataSet = this.removeOtherAnnotations(dataSet, attribute);
				dataSet = Filter.useFilter(dataSet, _trainerConvertToWordVector.get("at_"+attribute));
				dataSet.setClassIndex(this.getAnnotationPosition(dataSet, attribute));
				
				if (classifier.classifyInstance(dataSet.firstInstance()) == 0) {
					SecurityObjectiveAnnotation soa = new SecurityObjectiveAnnotation(attribute,SecurityImpact.LOW, SecurityMitigation.PREVENTION,false);
					result.annotations.add(soa);
				}
				
				result.distributions.put(attribute.toString(), classifier.distributionForInstance(dataSet.firstInstance()));
			}
			catch (Exception e) {
				System.out.println("WekaInterfacer.classify: "+e);
				return null;
	 ...
	 * @param soToTest if null, all are tested.  Otherwise just the objective is tested
	 * @return
	 */
	public ClassificationResult classifyAnnotationsWithNaiveBayesMultinomial(Instances originalDataSet, SecurityObjective soToTest) {
		return classifyAnnotations(_naiveBayesClassifierForAnnotations,originalDataSet,soToTest);
	}
	 ...
	 * @return
	 */
	public ClassificationResult classifyAnnotationsWithSMO(Instances originalDataSet, SecurityObjective soToTest) {
		return classifyAnnotations(_smoClassifierForAnnotations,originalDataSet,soToTest);
	}	
	
	public void generateInformationGainMatrixAnnotations(Instances completeDataSet, java.util.List<SecurityObjective> attributeList, int numberOfTerms) {
		
		java.util.HashMap<SecurityObjective, java.util.ArrayList<String>> topWords = new java.util.HashMap<SecurityObjective, java.util.ArrayList<String>>();
		...
		for (SecurityObjective so: attributeList) {
			...
			try {
				...
				topWords.put(so,topWordsForObjective);
			}
			catch (Exception e) {
				System.out.println(e);
			}
		...
		for (int i=0; i < numberOfTerms; i++) {
			...
			for (SecurityObjective so: attributeList) {
				...
				java.util.ArrayList<String> topWordsForObjective = topWords.get(so);
