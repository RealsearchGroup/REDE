...
public class DocumentFoldCreator {
	...
	public static int countNumberOfSentencesByAnnotation(java.util.ArrayList<Document> documents, SecurityObjective objective) {  //TODO: Should this include the test set?
		int result =0;
		
		for (Document d: documents) {
			if (d.sentence.hasSecurityObjectiveAnnotation(objective)) {
				result++;
			}
		}
		return result;
	...
	public static java.util.ArrayList<java.util.ArrayList<Document>> createFoldsByObjective(java.util.ArrayList<Document> documents, SecurityObjective objective, int numberOfFolds) {
		int numberOfTrainedSentences   = documents.size();
		int numberOfPrimarySentences    = countNumberOfSentencesByAnnotation(documents, objective);
		int numberOfNonPrimarySentences = numberOfTrainedSentences-numberOfPrimarySentences;
		double primaryClassificationRatio = numberOfPrimarySentences/((double) numberOfNonPrimarySentences);
		...
		for (int i=0;i<numberOfFolds;i++) {
			...
			while (numberOfElementsRemainingToPutinFold > 0) {
				...
				if (d.sentence.hasSecurityObjectiveAnnotation(objective) && expectNumberOfPrimaryLeft >0) {
					fold.add(d);
					expectNumberOfPrimaryLeft--;
					numberOfElementsRemainingToPutinFold--;
					unPlacedSentences.remove(indexToCheck);
				...
				else if (!d.sentence.hasSecurityObjectiveAnnotation(objective) && expectNumberOfNonPrimaryLeft >0) {
					fold.add(d);
					expectNumberOfNonPrimaryLeft--;
					numberOfElementsRemainingToPutinFold--;
					unPlacedSentences.remove(indexToCheck);
