...
public class WordVertex implements Serializable {
	...
	/**
	 * Using the path from element (which should be in a separate graph from the root node) to its root,
	 * find the matching element in the graph represented by the root.
	 * 
	 * @param element
	 * @return
	 */
	@JsonIgnore
	public List<WordVertex> getNodeFollowingRootPathFromOtherGraph( WordVertex element, boolean matchNodes, HashSet<String> knownItems) {
		List<WordVertex> result = new ArrayList<WordVertex>();
		List<Object> path = element.getPathToRoot();
		java.util.Collections.reverse(path);  // this now contains the path from this node to the other
		
		//WordVertex possibleResult = this;
		//I can probably simplify this by simply calling getNodeFollowingRootPathFromOtherGraph with a current index of -1.
		result.addAll(this.getNodeFollowingRootPathFromOtherGraph(path,-1, matchNodes, knownItems));
		
		/*
		if (path.size() > 1){
			WordEdge we = (WordEdge) path.get(1);
			
			List<WordEdge> childEdges = this.getChildRelationshipAll(we,true);
			for (WordEdge childEdge: childEdges) {
				//possibleResult = childEdge.getChildNode();
				result.addAll(childEdge.getChildNode().getNodeFollowingRootPathFromOtherGraph(path,1, matchNodes, knownItems));
			}
		}
		else {  //path was empty, just use the root node
			if (matchedNodeWhilefollowingPath(matchNodes,knownItems,this.getLemma(),element.getLemma(),this.getPartOfSpeech(),element.getPartOfSpeech())) {				
				result.add(this);
			}
		}
		*/
	...
	@JsonIgnore
	private static boolean matchedNodeWhilefollowingPath(boolean matchNodes,  HashSet<String> knownItems, String myLemma, String patternLemma, PartOfSpeech myPOS, PartOfSpeech patternPOS) {
		if (matchNodes) {
			if (patternLemma.equals(AccessControlPattern.WILDCARD_ANYTHING)) {
				return true;
			}
			else if (patternLemma.equals(AccessControlPattern.WILDCARD_SAME_ELEMENT)) {
				if (knownItems.contains(myLemma)) {
					return true;
				}
			}
			else if ((patternLemma.equals(AccessControlPattern.WILDCARD_SAME_PART_OF_SPEECH))) {
				if (patternPOS.equalsCollapsed(myPOS)) {
					return true;
				}
			}
			else {
				if (patternLemma.equals(myLemma)) {
					return true;
				}
			}
	...
	@JsonIgnore
	private List<WordVertex> getNodeFollowingRootPathFromOtherGraph( List<Object> path, int currentIndex, boolean matchNodes, HashSet<String> knownItems) {
		List<WordVertex> result = new ArrayList<WordVertex>();
		
		currentIndex++;
		if (currentIndex >= path.size()) { return result;}  // I'm at the end of the path, just return.  Shouldn't be called, but in place as a defensive mechanism
		
		WordVertex patternNode = (WordVertex) path.get(currentIndex);
		if (matchedNodeWhilefollowingPath(matchNodes, knownItems, this.getLemma(), patternNode.getLemma(), this.getPartOfSpeech(), patternNode.getPartOfSpeech())) {
			if (currentIndex +1 == path.size()) { // no more elements present, return this node
				result.add(this);
				return result;
			}
			else { // need to follow child relationships
				currentIndex++;
				WordEdge we = (WordEdge) path.get(currentIndex);
				
				List<WordEdge> childEdges = this.getChildRelationshipAll(we,true);
				for (WordEdge childEdge: childEdges) {
					//possibleResult = childEdge.getChildNode();
					result.addAll(childEdge.getChildNode().getNodeFollowingRootPathFromOtherGraph(path,currentIndex, matchNodes, knownItems));
				}							
			}
		}
		else {
	...
	private List<WordVertex> getNodeFollowingRootPathFromOtherGraph( List<Object> path, int currentIndex, boolean matchNodes, HashSet<String> knownItems) {
		...
		else {  // we are at the end of the path, just add ourselved and return
			result.add(this);
		}
